...
export class ClassDetailComponent implements OnInit {
  ...
  exportToPdf(): void {
    ...
    const className = this.classInfo?.subject || 'Class';
    const sectionName = this.classInfo?.section || 'Section';
    ...
    if (!printWindow) {
      alert('Pop-up blocker is preventing export. Please allow popups for this site.');
      return;
    }

    const examHeadersHtml = this.examNames.map(name => `<th>${name}</th>`).join('');
    const rowsHtml = this.students.map(student => {
      const scoresHtml = student.scores.map(s => `<td>${s.value}</td>`).join('');
      return `
        <tr>
          <td style="text-align: left; font-weight: 600; text-transform: uppercase;">${student.name}</td>
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>${className} (${sectionName}) - Gradesheet Report</title>
          <style>
            body {
            ...
            .header {
              margin-bottom: 30px;
              border-bottom: 2px solid #206a3d;
              padding-bottom: 15px;
            }
            .header h1 {
              margin: 0;
        ...
        <body>
          <div class="header">
            <h1>TSEK Gradesheet Report</h1>
            <p>Generated official summary of student performance</p>
          </div>
          
          <div class="meta-grid">
            <div>
              <strong>Class:</strong> ${className}<br>
              <strong>Section/Block:</strong> ${sectionName}
            </div>
            <div>
              <strong>Total Students:</strong> ${this.students.length}<br>
              <strong>Export Date:</strong> ${dateStr}
            </div>
          </div>
          ...
          <div class="footer">
            Report generated by TSEK AI Grading Assistant.
          </div>
  ...
  exportToJson(): void {
    ...
    const data = {
      className: this.classInfo?.subject,
      ...
      students: this.students.map(student => ({
        name: student.name,
        lrn: student.number,
        scores: student.scores.map((s, idx) => ({
          exam: this.examNames[idx],
          score: s.value
        }))
      }))
    };
    ...
    link.setAttribute('href', url);
    link.setAttribute('download', `${classNameClean}_${sectionNameClean}_Scores.json`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
